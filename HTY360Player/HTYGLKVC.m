...
//        NSLog(@"x--%f     y--%f       z--%f",cPitch,cRoll,cYaw);

      
      if (YES) {
          
//        modelViewMatrix = GLKMatrix4RotateY(modelViewMatrix, cRoll); // Up/Down axis
//        modelViewMatrix = GLKMatrix4RotateX(modelViewMatrix, cPitch);
...
//          NSLog(@"%f",cPitch);
//        modelViewMatrix = GLKMatrix4RotateX(modelViewMatrix, _fingerRotationX - cPitch);
//        modelViewMatrix = GLKMatrix4RotateY(modelViewMatrix, _fingerRotationY + cRoll);
//        modelViewMatrix = GLKMatrix4RotateZ(modelViewMatrix, cYaw);

          
//          NSLog(@"%f",_fingerRotationX);
          
          
          
//          modelViewMatrix = [MathTools getGLKMatrix4WithCRoll:-cRoll - _fingerRotationY cYaw:-cYaw cPitch:cPitch - _fingerRotationX];
          
          
          modelViewMatrix = [self getDeviceOrientationMatrix];
          modelViewMatrix = GLKMatrix4RotateZ(modelViewMatrix, cYaw);
...
-(GLKMatrix4) getDeviceOrientationMatrix{
    if(_isUsingMotion && [_motionManager isDeviceMotionActive]){
